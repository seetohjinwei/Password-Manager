...
class Passwords:
    ...
    def __init__(self, master: bytes, data: dict[str, dict[str, str]]) -> None:
        ...
        self.accounts: dict[str, Account] = {
            site: Account(site, d["un"], d["iv"], d["pw"]) for site, d in data.items()
        }
    ...
    if option == 1:
        ...
        if not accounts:
            ...
            for account in accounts:
                print(
                    f"{account} {Hash.decode_with_quotes(account.iv, account.hashed_password, master)}"
                )
